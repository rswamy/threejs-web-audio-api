...
class DiscoScene extends Scene2D {
	start() {
		...
		for (let i = 0; i < numberOfFlecks; i++) {
			...
			discoFleck.lineStyle(1, 0xffffff, 1);
	...
	audioTick(audioData) {
		...
		childrenArray.forEach(function (child) {
			if(childrenArray.indexOf(child) % 3 == 0) {
				child.scale.x = 1 + (volume * 10);
			}
			if(childrenArray.indexOf(child) % 7 == 0) {
				child.rotation += 0.01
			}
		});
	}
...
class StickScene extends Scene3D {
	start() {
		...
		scene.add(pivot);
		var rotation = { x: 0, y: 0, z:0 }; // Start at (0, 0)
		var tween = new TWEEN.Tween(rotation) // Create a new tween that modifies 'coords'.
			.to({ x: 0, y: Math.PI * 2, z: Math.PI * 2 }, 120000) // Move to this position in 30sec.
			.repeat( Infinity )
			.onUpdate(function() { // Called after tween.js updates 'coords'.
				// Move 'box' to the position described by 'coords' with a CSS translation.
				pivot.rotation.z = rotation.z;
				for(let i = 0; i < stickGroup.children.length; i++) {
					stickGroup.children[i].rotation.y = rotation.y * i;
				}
			})
